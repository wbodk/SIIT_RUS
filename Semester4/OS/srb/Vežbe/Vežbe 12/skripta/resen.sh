...
[[ -z "$K2_LOGFILE" ]] && read -p 'Unesite putanju do evidencije: ' K2_LOGFILE

if ! [[ -w "$K2_LOGFILE" ]]
then
    echo 'Nemate pravo pristupa datoteci' >&2
    exit 1
fi
...
do
    ...
    while [[ "$c" -lt 1 ]] || [[ "$c" -gt 100 ]]
    do
	read -p "$a * $b = " c || break 2
    done

    if [[ "$((a * b))" -eq "$c" ]]
    then
	echo 'Odgovor je tacan'
	echo "$a * $b = $c: tacno" >>"$K2_LOGFILE"
    else
	echo 'Odgovor je netacan'
	echo "$a * $b = $c: netacno" >>"$K2_LOGFILE"
    fi
done
