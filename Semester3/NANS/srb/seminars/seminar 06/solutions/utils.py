...
def are_assumptions_satisfied(model, x, y, p_value_thresh=0.01):
    ...
    is_linearity_found, p_value = linear_assumption(model, x_with_const, y, p_value_thresh, plot=False)
    autocorrelation, dw_value = independence_of_errors_assumption(model, x_with_const, y, plot=False)
    n_dist_type, p_value = normality_of_errors_assumption(model, x_with_const, y, p_value_thresh, plot=False)
    e_dist_type, p_value = equal_variance_assumption(model, x_with_const, y, p_value_thresh, plot=False)
    has_perfect_collinearity = perfect_collinearity_assumption(x, plot=False)
