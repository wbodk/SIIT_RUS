...
void show() {
	cout << "=======================" << endl;
	for (int i = 0;  i < 5; i++) {
		cout << "Philosopher " << i+1 << ":" << philosophers[i].displayState() << endl;
	}
	cout << "=======================" << endl;
...
void thread_philosopher(Philosopher& philosopher, Dining_table& dining_table) {
	...
	philosopher.setPhilosopherState(WAITING_LEFT_FORK);
	dining_table.take_fork(id);
	...
	philosopher.setPhilosopherState(WAITING_RIGHT_FORK);
	dining_table.take_fork(mod5(id+1));
